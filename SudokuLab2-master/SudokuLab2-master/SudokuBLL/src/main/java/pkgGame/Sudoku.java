...
public class Sudoku extends LatinSquare {
	...
	private void shuffleArray(int[] array) {
		...
		SecureRandom randArray = new SecureRandom();

		for (int i = 0; i < array.length; i++) {
			int randomPos = randArray.nextInt(array.length);
			int temp = array[i];
	...
	public void printPuzzle() {
		...
		for (int i = 0; i < this.getPuzzle.length; i++) {
			for (int j = 0; j < this.getPuzzle.length; i++) {
	...
	public void shuffleRegion(int r) {
		...
		for (i; i < iMax; i++) {
			for(j; j < jMax; j++) {
				this.getPuzzle()[i][j] = Region[number];
				number++;
			}
		}
	}

	public fillDiagonalRegions(){
		for(int i = 0; i < iSqrtSize; i++) {
			int Region = i*iSqrtSize+i;
